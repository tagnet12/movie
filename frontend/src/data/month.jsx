...
function month({setImgNo}){
  ...
  const yearChange = (year)=>{
    if(year == '년도별 선택'){ // 전체 영화 목록 관련 세팅
      ...
      if(selectedItem2 == '월별 선택'){   // 해당 년도의 영화목록 관련 세팅
        setSectionTitle2('');
        setSelectDate(year.substring(0,4)); // 년도만으로 필터링 (예: '2025')
      } else {                           // 해당 월의 영화목록 관련 세팅
        // 이미 월이 선택되어 있는 경우
  ...
  const monthChange = (months)=>{
    setSectionTitle2(months); setSelectedItem2(months); // 선택된 월의 정보 세팅
    setSelectDate(years + '.' + months.substring(0,2)); // 선택된 전체 정보 세팅 (년도.월)
  }

  // 선택한 장르에 대한 필터링 설정 ( 선택한 장르의 영화목록 검색 )
